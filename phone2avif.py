...
def process_one_image(args):
    ...
    try:
        ...
        subprocess.run([
            EXIFTOOL_PATH, "-overwrite_original", "-TagsFromFile", input_path,
            "-all:all", output_path
...
class App(TkinterDnD.Tk):
    ...
    def _setup_ui(self):
        ...
        self.guide_label = tk.Label(self.listbox, text="ここに対象となるフォルダをドラッグアンドドロップしてください", 
                                    bg="#fcfcfc", fg="gray")
        ...
        self.btn_start = tk.Button(self.action_frame, text="一括変換を開始する", command=self.start_thread, 
                                   bg="#28a745", fg="white", font=self.title_font, height=2)
        self.btn_start.pack(fill=tk.X)
        self.btn_stop = tk.Button(self.action_frame, text="変換を中止する", command=self.stop_conversion, 
                                  bg="#dc3545", fg="white", font=self.title_font, height=2)
    ...
    def show_license(self):
        ...
        messagebox.showinfo("ライセンス・権利情報", license_text)
    ...
    def run_conversion(self):
        ...
        try:
            input_threads = int(self.var_threads.get())
        ...
        with ProcessPoolExecutor(max_workers=num_threads) as executor:
            ...
            try:
                for _ in executor.map(process_one_image, tasks):
                    if not self.is_running: break
                    count += 1
